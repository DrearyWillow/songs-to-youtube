...
class SettingTemplate(Template):
    ...
    def safe_substitute(self, _: str | None, /, **kws: str) -> str:
        self.mapping: dict[str, str] = kws

        # Helper function for .sub()
        def convert(mo: Match[str]) -> str:
            named = mo.group("named") or mo.group("braced")
            if named is not None:
                return self.apply_args(named) or mo.group()
    ...
    def apply_args(self, named: str) -> str | None:
        for arg in named.split("|"):
            if arg in self.mapping:
                return str(self.mapping[arg])
            ...
            if len(arg) >= 2 and arg[0] == "<" and arg[-1] == ">":
                ...
                arg_ = arg[1:-1]
                if arg_ in self.mapping:
                    return sanitize_filename(str(self.mapping[arg_]))
                return "INVALID"
        return None
